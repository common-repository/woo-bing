...
class WooBing_Admin_Settings {
    ...
    function show_sub_tab_page( $page, $tab, $subtab ) {
        ...
        if( empty( $menu[$tab]['submenu'] ) && count( $menu[$tab]['submenu'] ) ) {
            ...
            $path = apply_filters( 'woobing_subtab_path', $path, $page, $tab, $subtab );
	...
	function select_field( $name, $element ) {
        ...
        $element['required']    = ( isset( $element['extra']['data-woobing_required'] ) &&  ( $element['extra']['data-woobing_required'] === true ) ) ? '*' : '';
        ...
        $html .= sprintf( '</select>' );
        $html .= sprintf( '<span class="woobing-clear"></span><span class="description"> %s</span>', $element['desc'] );

        ob_start();
        echo $this->wrap_start( $element ); 
       	echo '<div id="woobing-select" class="woobing-form-field">';
        echo $html;

        //do_action( 'woobing_select_' . $element['action_hook'] );
        echo '</div>';
    ...
    function multiple_select_field( $name, $element ) {
        ...
        echo '<div id="woobing-multi-select" class="woobing-form-field">';
        echo $html;
    ...
    function text_field( $name = '', $element ) {
        ...
        $element_required    = ( isset( $element['extra']['data-woobing_required'] ) &&  ( $element['extra']['data-woobing_required'] === true ) ) ? '*' : '';
        ...
        $this->wrap_start( $element );
        ?>
        <div class="woobing-form-field woobing-text">
            ...
            <span class="woobing-clear"></span><span class="description"><?php echo $element_desc; ?></span>
    ...
    function radio_field( $name = '', $element ) {
        ...
        foreach( $fields as $field ) {
            ...
            $id         = isset( $field['id'] ) ? esc_attr( $field['id'] ) : 'woobing_' .$name .'_'. $value;
            ...
            $html .= sprintf( '<label class="woobing-radio" for="%1s">%2s</label>', $id, $label );

        }

        $desc = isset( $element['desc'] ) ? esc_attr( $element['desc'] ) : '';
        $html .= sprintf( '<span class="woobing-clear"></span><span class="description">%s</span>', $desc );
        ob_start();
        $this->wrap_start( $element );
        echo '<div id="woobing-radio" class="woobing-form-field">';
        echo $html;
        echo '</div>';
        $this->wrap_close( $element );
        return ob_get_clean();
    ...
    function checkbox_field( $name = '', $element ) {
        ...
        foreach( $fields as $field ) {
            ...
            $html .= sprintf( '<label class="woobing-checkbox" for="%1s">%2s</label>', $id, $label );
        ...
        ob_start();
        $this->wrap_start( $element );
        echo '<div class="woobing-form-field">';
        echo $html;
        echo '</div>';
        $this->wrap_close( $element );
        return ob_get_clean();
    ...
    function html_field( $name, $element ) {
        ...
        ob_start();
            $this->wrap_start( $element );
            if ( !empty( $text ) ) {
                echo '<div class="woobing-form-field woobing-html">';
                echo $text;
                echo '</div>';
            }
            $this->wrap_close( $element );
