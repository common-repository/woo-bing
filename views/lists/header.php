...
<h2 class="nav-tab-wrapper">
    ...
    foreach ( $menu as $tab_key => $tab_event ) {
        ...
        $url = woobing_tab_menu_url( $tab_key );
...
?>
<h3 class="woobing-sub-nav">
    <ul class="woobing-subsubsub">
        <?php
            $sub_menu = array();

            foreach ( $menu[$tab]['submenu'] as $sub_key => $sub_event ) {
                $sub_active = ( $sub_key == $subtab ) ? 'woobing-sub-current' : '';
                $sub_event['id'] = isset( $sub_event['id'] ) ? $sub_event['id'] : '';
                $sub_url = woobing_subtab_menu_url( $tab, $sub_key );
